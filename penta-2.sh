...
deploy_new_service() {
  ...
  while true; do
      ...
      case $RES_MODE in
          1)
              ...
              echo "1) Qwiklabs Safe: 1Gi RAM + 1 vCPU (Min: 1, Max: 2, Concurrency: 250) ✅"
              echo "2) Balanced:      2Gi RAM + 2 vCPU (Min: 1, Max: 2, Concurrency: 400)"
              echo "3) High Performance: 4Gi RAM + 2 vCPU (Min: 1, Max: 2, Concurrency: 500)"
              ...
              case $AUTO_CHOICE in
                  1) 
                    MEMORY="1Gi"; CPU="1"
                    MIN_INST=1; MAX_INST=2; CONCURRENCY=250; TIMEOUT=3600
                    ;;
                  2) 
                    MEMORY="2Gi"; CPU="2"
                    MIN_INST=1; MAX_INST=2; CONCURRENCY=400; TIMEOUT=3600
                    ;;
                  3) 
                    MEMORY="4Gi"; CPU="2"
                    MIN_INST=1; MAX_INST=2; CONCURRENCY=500; TIMEOUT=3600
                    ;;
                  *) 
                    MEMORY="1Gi"; CPU="1"
                    MIN_INST=1; MAX_INST=2; CONCURRENCY=250; TIMEOUT=3600
  ...
  # ==============================================
  # XRAY CONFIG — Perfectly Tuned for Trojan & Mux
  # ==============================================
  cat > config.json <<'EOF'
{
