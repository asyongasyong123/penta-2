...
# =================================================================
# 🚀 GCP-XRAY ULTIMATE MULTI-ENGINE DEPLOYER (Qwiklabs Optimized)
# ✅ ENGINES: OPENRESTY, ENVOY, HAPROXY, CADDY, SING-BOX
# ✅ FEATURES: Supervisord, Anti-DDoS, Auto Log Cleaner
...
deploy_new_service() {
  ...
  echo -e "${CYAN}=========================================${NC}"
  echo "1) OpenResty          - [Anti-DDoS + Lua Tuned]"
  ...
  echo -e "\n${CYAN}=========================================${NC}"
  echo -e "${GREEN}      RESOURCE CONFIG MODE (Qwiklabs Safe)${NC}"
  echo -e "${CYAN}=========================================${NC}"
  echo -e "${GREEN}1) AUTO PRESETS  |  Safe & Optimized (Qwiklabs Compliant)${NC}"
  ...
  while true; do
      ...
      case $RES_MODE in
          1)
              ...
              echo "3) High Boost:     4Gi RAM + 2 vCPU (Min: 1, Max: 3, Concurrency: 1000)"
  ...
  # ==============================================
  # XRAY CONFIG — Original Credentials Kept
  # Added: /xhttp & /httpupgrade for NetMod + Mux stability
  # ==============================================
  cat > config.json <<'EOF'
{
...
EOF
  ...
  # ==============================================
  # 1. OPENRESTY ENGINE (WITH ANTI-DDOS)
  # ==============================================
        ...
        typed_config:
          ...
          route_config:
            ...
            - name: local_service
              ...
              - match: { prefix: "/xhttp" }
                route: { cluster: xhttp_cluster, timeout: 3600s }
              - match: { prefix: "/httpupgrade" }
                route: { cluster: httpupgrade_cluster, timeout: 3600s, upgrade_configs: [{ upgrade_type: "websocket" }] }
              - match: { prefix: "/" }
...
frontend main
    ...
    use_backend xhttp_backend if is_xhttp
    use_backend httpupgrade_backend if is_httpupgrade
